package com.gui.swing.Controller.menu.mode;
...
public class ToolBarAccentColor extends JPanel {
    ...
    private void init() {
        ...
        for (int i = 0; i < accentColorNames.length; i++) {
            ...
            // Kiểm tra xem màu sắc có null hay không trước khi sử dụng
            if (accentColor != null && colorForKey != null) {
                tbutton.setSelected(accentColor.equals(colorForKey));
            } else {
                // Xử lý trường hợp một hoặc cả hai giá trị là null
                // Bạn có thể làm gì đó tại đây, ví dụ: sử dụng một màu mặc định
                tbutton.setSelected(false); // hoặc true, tùy thuộc vào yêu cầu của bạn
            }
